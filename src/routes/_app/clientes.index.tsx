...
import { data as dataBR, mascaraDocumento, mascaraTelefone, reais, rotulo, soDigitos } from "@/lib/formato";
...
export const Route = createFileRoute("/_app/clientes/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Lista de clientes pessoa física e jurídica com documentos, contatos, imóveis e valores em aberto." },
      { property: "og:title", content: "Clientes | CRM de Topografia" },
      { property: "og:description", content: "Clientes PF e PJ com documentos, contatos e carteira vinculada." },
...
function Pagina() {
  ...
  const resumoQuery = useQuery({
    ...
    queryFn: async (): Promise<Resumo> => {
      const [imoveis, orcamentos, oportunidades, mensagens] = await Promise.all([
        ...
        supabase.from("mensagens").select("oportunidade_id, criado_em").order("criado_em", { ascending: false }).limit(3000),
      ]);
      ...
      for (const item of (imoveis.data ?? []) as { cliente_id: string | null }[]) {
        if (item.cliente_id) resumo.imoveis[item.cliente_id] = (resumo.imoveis[item.cliente_id] ?? 0) + 1;
      }
      for (const item of (orcamentos.data ?? []) as { cliente_id: string | null; total: number | null; status: string | null }[]) {
        if (!item.cliente_id) continue;
        if (["aprovado", "recusado", "expirado"].includes(item.status ?? "")) continue;
        resumo.aberto[item.cliente_id] = (resumo.aberto[item.cliente_id] ?? 0) + Number(item.total ?? 0);
      }
      const oportunidadeCliente = new Map<string, string>();
      for (const item of (oportunidades.data ?? []) as { id: string; cliente_id: string | null; criado_em: string | null }[]) {
        if (item.cliente_id) {
          oportunidadeCliente.set(item.id, item.cliente_id);
          if (item.criado_em && !resumo.ultimo[item.cliente_id]) resumo.ultimo[item.cliente_id] = item.criado_em;
        }
      }
      for (const item of (mensagens.data ?? []) as { oportunidade_id: string | null; criado_em: string | null }[]) {
  ...
  const filtrados = useMemo(() => {
    ...
    return (clientes ?? []).filter((c) => {
      ...
      const alvo = `${c.nome} ${c.nome_fantasia ?? ""} ${c.cidade ?? ""} ${c.uf ?? ""}`.toLowerCase();
      if (alvo.includes(termo)) return true;
      if (digitos.length === 0) return false;
      return soDigitos(c.documento ?? "").includes(digitos) || soDigitos(c.telefone ?? "").includes(digitos);
    });
  }, [clientes, busca, tipo]);
    ...
    <section className="space-y-4">
      ...
      <BarraFerramentas>
        <div className="relative min-w-64 flex-1">
          ...
          <Input value={busca} onChange={(e) => setBusca(e.target.value)} placeholder="Buscar por nome, documento ou telefone" className="h-11 rounded-full border pl-11" />
        </div>
        <div className="seg">
          {(["todos", "pf", "pj"] as const).map((item) => (
            <button key={item} type="button" data-ativo={tipo === item} onClick={() => setTipo(item)} className="seg-item">
      ...
      <div className="grade-3 grid grid-cols-1 gap-3 sm:grid-cols-3">
        <CartaoIndicador icone={Users} valor={filtrados.length} rotulo="Clientes no filtro" apoio={`de ${(clientes ?? []).length} cadastrados`} destino="/imoveis" />
        <CartaoIndicador icone={Building2} valor={totalImoveis} rotulo="Imóveis vinculados" apoio="na carteira filtrada" destino="/imoveis" />
        <CartaoIndicador icone={CircleDollarSign} valor={reais(valorAberto)} rotulo="Em aberto" apoio="orçamentos em decisão" tom="atencao" destino="/orcamentos" />
      </div>

      <Painel titulo="Carteira de clientes" icone={Users} acao={<span className="text-sm font-bold text-muted-foreground">{filtrados.length} itens</span>}>
        {error ? (
          <p className="text-base font-bold text-destructive">Não foi possível carregar os clientes.</p>
        ) : isPending ? (
                ...
                {filtrados.map((c) => (
                  <tr key={c.id} className="border-b border-border last:border-0">
                    <td className="px-3 py-3">
                      <Link to="/clientes/$id" params={{ id: c.id }} className="flex items-center gap-3 text-foreground hover:text-primary">
                        <Avatar className="size-9 border border-border"><AvatarFallback>{inicial(c.nome)}</AvatarFallback></Avatar>
                        <span className="min-w-0">
                          <span className="block truncate font-extrabold">{c.nome}</span>
                          {c.nome_fantasia ? <span className="block truncate text-sm font-semibold text-muted-foreground">{c.nome_fantasia}</span> : null}
                        </span>
                      </Link>
                    </td>
                    <td className="px-3 py-3 text-sm font-semibold text-muted-foreground">{c.documento ? mascaraDocumento(c.documento, c.tipo ?? "pf") : "—"}</td>
                    <td className="px-3 py-3 text-sm font-semibold text-foreground">{c.telefone ? mascaraTelefone(c.telefone) : "—"}</td>
                    <td className="px-3 py-3 text-center text-sm font-extrabold text-foreground">{resumo?.imoveis[c.id] ?? 0}</td>
                    <td className="px-3 py-3 text-right text-sm font-extrabold text-foreground">{reais(resumo?.aberto[c.id] ?? 0)}</td>
                    <td className="px-3 py-3 text-sm font-semibold text-muted-foreground">{resumo?.ultimo[c.id] ? dataBR(resumo.ultimo[c.id]) : "—"}</td>
                    <td className="px-3 py-3">
                      <Badge variant="outline" className="gap-1.5 rounded-full border-border bg-card px-2.5 py-1 text-xs text-foreground">
                        <span className={`size-2 rounded-full ${(c.tipo ?? "pf") === "pj" ? "bg-primary" : "bg-muted-foreground"}`} aria-hidden />
                        {rotulo(c.tipo ?? "pf")}
                      </Badge>
                    </td>
                  </tr>
