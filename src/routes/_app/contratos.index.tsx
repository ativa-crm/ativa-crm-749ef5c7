...
export const Route = createFileRoute("/_app/contratos/")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Contratos com busca, vigência, assinatura, alerta e documentos em PDF.",
      },
      { property: "og:title", content: "Contratos | CRM de Topografia" },
      {
        property: "og:description",
        content: "Controle de contratos, vencimentos, assinatura e PDFs.",
      },
...
function Pagina() {
  ...
  const query = useQuery({
    ...
    queryFn: async (): Promise<Contrato[]> => {
      const { data, error } = await supabase
        .from("contratos")
        .select(
          "id, numero, status, valor, criado_em, vigencia_ate, assinatura_status, pdf_url, clientes(nome), imoveis(nome)",
        )
  ...
  const filtrados = useMemo(() => {
    ...
    return (query.data ?? []).filter((c) =>
      `${c.numero ?? ""} ${um(c.clientes)?.nome ?? ""} ${um(c.imoveis)?.nome ?? ""} ${rotulo(c.status)}`
        .toLowerCase()
        .includes(termo),
    );
  }, [query.data, busca]);
  ...
  const valorAtivo = filtrados
    .filter((c) => c.status === "ativo" || c.status === "fechado")
    ...
    <section className="space-y-4">
      ...
      <BarraFerramentas>
        <div className="relative min-w-64 flex-1">
          ...
          <Input
            ...
            placeholder="Buscar por contrato, cliente, imóvel ou status"
      ...
      <div className="grade-3 grid grid-cols-1 gap-3 sm:grid-cols-3">
        ...
        <CartaoIndicador
          ...
          apoio="renovar ou encerrar"
          tom={aVencer.length ? "atencao" : "neutro"}
          destino="/contratos"
        />
        <CartaoIndicador
          icone={FileCheck2}
          ...
          apoio={reais(valorAtivo)}
          tom={semAssinatura.length ? "critico" : "neutro"}
          destino="/contratos"
        />
      </div>

      {alerta ? (
        <div className="flex items-start gap-3 rounded-lg border border-destructive/40 bg-card p-4">
          <AlertTriangle className="mt-1 size-5 shrink-0 text-destructive" />
          <p className="text-base font-bold text-foreground">
            {alerta.numero ? `Contrato ${alerta.numero}` : "Contrato"} ·{" "}
            {um(alerta.clientes)?.nome ?? "cliente não informado"}{" "}
            {statusAssinatura(alerta) !== "assinado"
              ? "está aguardando assinatura."
              : "está perto do vencimento."}
          </p>
        </div>
      ) : null}
      ...
      >
        {query.isPending ? (
          ...
          <p className="text-base font-medium text-muted-foreground">Nenhum contrato encontrado.</p>
        ) : (
          <Tabela>
                  ...
                  return (
                    <tr key={c.id} className="border-b border-border last:border-0">
                      <td className="px-3 py-3">
                        ...
                        >
                          {c.numero ? `Contrato nº ${c.numero}` : "Contrato"}
                        </Link>
                        <span className="block text-xs font-bold uppercase text-muted-foreground">
                          {rotulo(c.status) || "sem status"}
                        </span>
                      </td>
                      ...
                      <td className="px-3 py-3 text-sm font-semibold text-muted-foreground">
                        {c.vigencia_ate
                          ? `${dataBR(c.vigencia_ate)}${dias !== null ? ` · ${dias < 0 ? "vencido" : `${dias} d`}` : ""}`
                          : "—"}
                      </td>
                      <td className="px-3 py-3">
                        <BadgeContrato contrato={c} />
                      </td>
                      <td className="px-3 py-3">
                        {c.pdf_url ? (
                          <Badge className="rounded-full">PDF pronto</Badge>
                        ) : (
                          <Badge
                            variant="outline"
                            className="gap-1.5 rounded-full border-border bg-card px-2.5 py-1 text-xs text-foreground"
