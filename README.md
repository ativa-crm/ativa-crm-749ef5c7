...
O contrato visual versionado está em `DESIGN.md`. A biblioteca integrada vive em `design-system/`, com componentes em `design-system/components/`, tokens em `design-system/tokens/`, guidelines em `design-system/guidelines/`, assets em `design-system/assets/` e referências HTML em `design-system/referencia/`. Componentes novos devem ser registrados em `design-system/catalog.ts` e usar tokens, sem valores de cor, fonte, espaçamento ou raio hardcoded. A showcase está disponível em `/design-system`.
